...
BEGIN
  ...
  BEGIN
    ...
    (
      ...
      WHERE (@DepartmentId IS NULL OR [DepartmentId] = @DepartmentId) AND (@CategoryId IS NULL OR [CategoryId] = @CategoryId) AND (@EncoderId IS NULL OR [EncoderId] = @EncoderId) AND (@CurrencyId IS NULL OR [CurrencyId] = @CurrencyId) AND (@AssignedToUserId IS NULL OR [AssignedToUserId] = @AssignedToUserId) AND (@VendorId IS NULL OR [VendorId] = @VendorId) 
    )
