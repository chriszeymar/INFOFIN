/****** Object:  StoredProcedure [dbo].[zgen_Actuals_InsUpd] ******/
-- ===================================================================
-- Description    : Insert Update Actuals
-- ===================================================================

CREATE   PROCEDURE [dbo].[zgen_Actuals_InsUpd]
  (@DepartmentId INT,@CategoryId INT,@Year INT,@Amount DECIMAL(18, 2),@Id INT=NULL,@Month INT=NULL,@RetMsg NVARCHAR(MAX) OUTPUT)
AS
BEGIN
  ...
  BEGIN TRY
    ...
    BEGIN
      INSERT INTO [dbo].[Actuals]
        ([DepartmentId],[CategoryId],[Year],[Amount],[Month])
      VALUES
        (@DepartmentId,@CategoryId,@Year,@Amount,@Month);
      SELECT * FROM [dbo].[Actuals] WHERE [Id] = SCOPE_IDENTITY();
    END
  ELSE
    BEGIN
      UPDATE [dbo].[Actuals]
        SET [DepartmentId]=@DepartmentId,[CategoryId]=@CategoryId,[Year]=@Year,[Amount]=@Amount,[Month]=@Month,[UpdateDT]=GETDATE()
        WHERE ([Id] = @Id);
