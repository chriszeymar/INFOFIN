...
CREATE   PROCEDURE [dbo].[zgen_Currency_InsUpd]
  (@Code NVARCHAR(10),@ExchangeRateToUSD DECIMAL(18, 6),@Id INT=NULL,@IsActive BIT=1,@RetMsg NVARCHAR(MAX) OUTPUT)
AS
BEGIN
  ...
  BEGIN TRY
    ...
    BEGIN
      INSERT INTO [dbo].[Currency]
        ([Code],[ExchangeRateToUSD],[IsActive])
      VALUES
        (@Code,@ExchangeRateToUSD,@IsActive);
  ...
  ELSE
    BEGIN
      UPDATE [dbo].[Currency]
        SET [Code]=@Code,[ExchangeRateToUSD]=@ExchangeRateToUSD,[IsActive]=@IsActive,[UpdateDT]=GETDATE()
        WHERE ([Id] = @Id);
